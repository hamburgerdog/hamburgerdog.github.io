...
tags: 前端 源码
remark: 'React Reconciliation 的核心过程，包括新旧 Fiber 树对比、effect 标记、key 的作用、beginWork 与 completeUnitOfWork 的遍历机制，以及 double buffering 双缓冲机制。中断与恢复机制详见《React 中断与恢复》'
---
...
1. **状态错位**：index 变化后，React 会复用错误的 Fiber 节点，导致状态错位
2. **性能问题**：无法正确复用节点，导致不必要的更新和重新渲染
3. **副作用混乱**：useEffect、useLayoutEffect 等副作用可能绑定到错误的节点
...
1. **统一 key 提取**：无论 key 是字符串、数字还是其他类型，统一处理
2. **降级处理**：如果没有显式 key，使用索引作为降级方案
3. **性能优化**：通过 key 快速匹配新旧节点，避免遍历查找
...
关于中断与恢复的详细机制，包括状态保存、优先级匹配处理、bailout 机制等，请参考：[React 中断与恢复](./react-中断与恢复)。
