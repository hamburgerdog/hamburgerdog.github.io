...
Portal 中的事件会**冒泡到 React 树**，而不是 DOM 树。这是 React 事件系统的核心特性。
...
- **DOM 冒泡**：`button` → `div.modal` → `body` → `document`
- **React 冒泡**：`button` → `Modal` → `div`（App 的 div）→ `handleClick` 触发 ✅
...
React **不会**直接在 DOM 元素上绑定事件监听器，而是：
...
React 使用**事件委托**（Event Delegation）机制：

1. **统一委托**：所有事件都委托到根容器（通常是 `document` 或 `root`）
