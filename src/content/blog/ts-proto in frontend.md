...
同时，又有 `service` 来定义通信接口的结构，我们可以从中提取出接口地址、描述、通信方式等关键信息，通过 TS 可以拼接成如下代码：
...
- JSDoc 的设想
  ...
  拼装 JSDoc 的简易程度能比 TS 在轻一个量级，在类型提示方面也相差无几。但还需要把 upper 和 lower 拆离开。

- 既然有 openAPI ，或许可以借用 protobuf -> openAPI -> restfulAPI(TS/JS)

  这个没有实地研究过，应该是可行的。

- 使用替换正则表达式实现，使用 TypeScript AST 解析。
